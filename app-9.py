...
if result is None:
    ...
    change_card_class = "card-buy" if price_change >= 0 else "card-sell"

    pc1, pc2 = st.columns([1, 3])
    with pc1:
        st.markdown(
            f"""
            <div class="signal-card {change_card_class}">
                <div class="card-title">{r_ticker} Current Price</div>
                <div class="card-value {change_color}">{format_price(last_close, r_ticker)}</div>
                <div style="margin-top:6px; font-size:0.85rem; font-weight:600; color:{'#12965a' if price_change >= 0 else '#c62828'};">
                    {change_sign}{format_price(price_change, r_ticker)} ({change_sign}{price_change_pct:.2f}%)
                </div>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    with pc2:
        st.markdown(
            f"""
            <div style="font-size:0.85rem; color:#5a6b85; padding-top:10px;">
                Last updated candle: <b>{df.index[-1].strftime('%Y-%m-%d %H:%M')}</b> ({r_timeframe} timeframe)<br>
                Session High: <b>{format_price(float(df['High'].iloc[-1]), r_ticker)}</b> &nbsp;|&nbsp;
                Session Low: <b>{format_price(float(df['Low'].iloc[-1]), r_ticker)}</b>
            </div>
            """,
            unsafe_allow_html=True,
        )
